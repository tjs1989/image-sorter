...
pytest tests/test_pull_from_android.py   # run one file
pytest -k recurs                         # run tests matching a keyword
```

`pytest.ini` sets `pythonpath = .` so tests can import the project's top-level packages (`pull`, `sort`, `system_operations`, etc.) without any package install.

```bash
python main.py pullfromandroid -f=/abs/path   # adb pull DCIM/Pictures/Movies/Music into folder
...
`pullfromandroid` requires `adb` on PATH (`brew install --cask android-platform-tools`) and a single authorized device. Source paths on the device are configured via `android_source_paths` in `system_config.yaml`.
...
2. **Command classes** — `sort/sort_files.py::SortFiles`, `delete/delete_files.py::DeleteFiles`, and `pull/pull_from_android.py::PullFromAndroid`. Each takes a folder path, instantiates its own `FileOperations`/`FolderOperations` where needed, and exposes a small public API the CLI calls.
...
- **Sort uses `mtime`, not EXIF.** `FileOperations.get_file_last_modified_details` reads `os.stat(...).st_mtime`. Files copied/restored from backups will sort by the copy date, not the photo date. `adb pull` preserves mtime, so the `pullfromandroid` → `sort` flow keeps original capture dates.
